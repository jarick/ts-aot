...
impl ExprConverter {
    ...
    fn try_date_static_method_dispatch(
        &mut self,
        callee: &HirCallee,
        args: &[HirExpr],
        ty: TypeId,
    ...
    fn try_date_instance_method_dispatch(
        &mut self,
        callee: &HirCallee,
        args: &[HirExpr],
        ty: TypeId,
        ...
        if let Some(owner_ty) = hir_expr_type_id(date_method_owner)
            && !matches!(types.resolve(owner_ty), Some(Type::Date))
        {
            return None;
        }
...
fn type_label(types: &TypeTable, ty: TypeId) -> String {
    match types.resolve(ty) {
...
impl ExprConverter {
    ...
    fn try_symbol_static_method_dispatch(
        &mut self,
        callee: &HirCallee,
        args: &[HirExpr],
        ty: TypeId,
