...
fn array_method_call_with_arg(field_name: &str, arg: HirExpr) -> HirExpr {
    HirExpr::Call {
        callee: HirCallee::Indirect(Box::new(HirExpr::Field {
            owner: Box::new(HirExpr::Global {
...
fn math_method_call_with_arg(field_name: &str, arg: HirExpr) -> HirExpr {
    HirExpr::Call {
        callee: HirCallee::Indirect(Box::new(HirExpr::Field {
            owner: Box::new(HirExpr::Global {
...
fn math_method_call_with_2_args(field_name: &str, arg1: HirExpr, arg2: HirExpr) -> HirExpr {
    HirExpr::Call {
        callee: HirCallee::Indirect(Box::new(HirExpr::Field {
            owner: Box::new(HirExpr::Global {
                name: Atom::new_inline("Math"),
...
fn math_max_with_three_args_emits_math_max_with_three_mir_args() {
    ...
    if let HirExpr::Call { ty, .. } = &mut call {
        *ty = unit_ty();
    }
    let _mir = c.convert_expr(
        &call,
...
fn string_method_call_with_args(field_name: &str, args: Vec<HirExpr>) -> HirExpr {
    HirExpr::Call {
        callee: HirCallee::Indirect(Box::new(HirExpr::Field {
            owner: Box::new(HirExpr::Global {
                name: Atom::new_inline("String"),
                ty: unit_ty(),
                span: Span::default(),
            }),
            field: FieldId::from_raw(0),
...
fn string_instance_method_call_with_args(
    owner: HirExpr,
    field_name: &str,
    args: Vec<HirExpr>,
) -> HirExpr {
...
fn string_index_of_call_with_one_arg_emits_op_with_default_from_index_zero() {
    ...
    let default_present = stmt
        .expect("checked is_some above")
        .iter()
        .any(|a| matches!(a, MirExpr::Int { value: 0, .. }));
    assert!(
        default_present,
        "fromIndex=0 must be synthesized for 1-arg form, got {stmt:?}"
    );
    assert!(
        matches!(mir, MirExpr::Local(_)),
...
#[test]
fn array_from_with_object_literal_negative_length_emits_e0406() {
    let mut c = ExprConverter::new();
    let out = &mut Vec::new();
    let mut cx = ctx();
    let mut types = TypeTable::new();
    ...
    assert_eq!(
        e0406_count,
        1,
        "Array.from({{length: -1}}) must emit E0406 (negative length is invalid), got {:?}",
        cx.diagnostics()
    );
    assert!(
        matches!(mir, MirExpr::Unit),
    ...
    assert_eq!(
        create_count, 0,
        "Array.from({{length: -1}}) must NOT emit ArrayCreateWithLen (validation rejects upfront), got: {out:?}"
    );
}
