...
#[test]
fn convert_program_emits_collision_for_qualified_and_literal_underscore_names() {
    let mut prog = HirProgram::new(ModuleId::from_raw(0));
    prog.push_decl(HirDecl::Namespace {
        name: Atom::new_inline("outer"),
        members: vec![HirDecl::Function(HirFunction {
            name: Atom::new_inline("ns_identity"),
            params: Vec::new(),
            ret: unit_ty(),
            throws: None,
            body: vec![HirStmt::Return { value: None }],
    ...
    prog.push_decl(HirDecl::Function(HirFunction {
        name: Atom::new_inline("outer__ns_identity"),
        params: Vec::new(),
        ret: unit_ty(),
        throws: None,
        body: vec![HirStmt::Return { value: None }],
    ...
    assert_eq!(
        e0503.len(),
        1,
        "expected exactly one E0503 for qualified-vs-literal name collision, got: {:?}",
        cx.diagnostics()
    );
}

#[test]
fn convert_program_qualified_and_literal_underscore_names_get_distinct_mir_names_via_disambiguation()
 {
    let mut prog = HirProgram::new(ModuleId::from_raw(0));
    prog.push_decl(HirDecl::Namespace {
        name: Atom::new_inline("outer"),
    ...
    assert_eq!(
        functions.len(),
        2,
        "both functions must be emitted to MIR after disambiguation, got: {:?}",
        functions
            .iter()
            .map(|f| f.name.as_str())
            .collect::<Vec<_>>()
    ...
    let ns_fn = functions
        .iter()
        .find(|f| f.name == Atom::new_inline("outer::ns_identity"))
        .expect(
            "namespace function must be emitted with its original mir_name `outer::ns_identity`",
        );
    let literal_fn = functions
        .iter()
        .find(|f| f.name == Atom::new_inline("outer__ns_identity_2"))
        .expect("literal-underscore function must be disambiguated to `outer__ns_identity_2`");
    assert_ne!(
        ns_fn.id, literal_fn.id,
        "namespace and disambiguated literal functions must have distinct FunctionIds; \
         got ns_fn.id={:?}, literal_fn.id={:?}",
        ns_fn.id, literal_fn.id
    );
    assert_eq!(
        ns_fn.export_name.as_deref(),
        Some("outer::ns_identity"),
        "namespace function's export_name must be the ORIGINAL qualified name, got: {:?}",
        ns_fn.export_name
    );
    assert_eq!(
        literal_fn.export_name.as_deref(),
        Some("outer__ns_identity"),
        "literal-underscore function's export_name must be the ORIGINAL name (NOT the \
         disambiguated `outer__ns_identity_2`) to preserve host dispatch, got: {:?}",
        literal_fn.export_name
    );
    let e0503: Vec<_> = cx
        .diagnostics()
        .iter()
    ...
    assert_eq!(
        e0503.len(),
        1,
        "pre-assign pass must still emit exactly one E0503 collision diagnostic even though \
         the convert pass disambiguated the second function, got: {:?}",
        cx.diagnostics()
    );
}

#[test]
fn convert_program_duplicate_class_consumes_method_function_ids_for_alignment() {
    use ts_aot_ir_hir::{HirClass, HirParam};
    let mut prog = HirProgram::new(ModuleId::from_raw(0));
    prog.push_decl(HirDecl::Class(HirClass {
        name: Atom::new_inline("Foo"),
    ...
    assert_eq!(
        structs.len(),
        1,
        "duplicate class `Foo` must not emit a second struct; got: {:?}",
        structs.iter().map(|s| s.name.as_str()).collect::<Vec<_>>()
    );
    let bar = mir
        .functions()
        .find(|f| f.name == Atom::new_inline("bar"))
        .expect("free function `bar` must be present in MIR after duplicate-class skip");
    assert_eq!(
        bar.id,
        FunctionId::from_raw(2),
        "FunctionId alignment must be preserved across the duplicate-class skip: \
         Foo::m consumes id 0, duplicate Foo::m's id (reserved 1 by pre_assign) must be \
         consumed by skip_class_method_ids so `bar` gets id 2. \
         Pre-fix bug: convert_decl::Class returned None without skipping the method ids, so \
         `bar` would have been allocated id 1 and the next pre-assigned declaration would \
         see a misaligned counter. Got bar.id={:?}",
        bar.id
    );
}
