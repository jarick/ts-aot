...
fn reject_nested_generator_decls(
    ...
    impl NestedGeneratorRejector<'_> {
        fn visit_decl(&mut self, decl: &HirDecl, report: bool) {
            match decl {
                HirDecl::Function(f) => {
                    if report && f.is_generator {
                ...
                HirDecl::Class(c) => {
                    for cm in &c.methods {
                        if report && cm.is_generator {
                            let span = first_body_span(&cm.body).unwrap_or_default();
                            report_nested_generator(self.ctx, self.stats, cm.name.as_str(), span);
                        }
                        self.visit_block(&cm.body);
                    }
                }
                HirDecl::Namespace { members, .. } => {
                    reject_nested_decl_list(members, self, report);
                }
                HirDecl::Global {
                    init: Some(expr), ..
    ...
    fn reject_nested_decl_list(
        members: &[HirDecl],
        rejector: &mut NestedGeneratorRejector<'_>,
        report: bool,
    ) {
        for m in members {
            rejector.visit_decl(m, report);
        }
    }
    let mut rejector = NestedGeneratorRejector { ctx, stats };
    for d in decls {
        rejector.visit_decl(d, false);
    }
}
