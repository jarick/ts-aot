...
#[test]
fn bigint_shl_by_negative_count_throws_range_error() {
    let result = std::panic::catch_unwind(|| {
        let _ = __ts_aot_bigint_shl(&b("42"), &b("-1"));
    });
...
#[test]
fn bigint_shr_by_negative_count_throws_range_error() {
    let result = std::panic::catch_unwind(|| {
        let _ = __ts_aot_bigint_shr(&b("42"), &b("-1"));
    });
    assert!(result.is_err());
    let msg = result.unwrap_err();
    let msg_str = msg
